...
cbtsv="${RUN}.CBcounts.tsv"
cbsumtsv="${RUN}.CBsums.tsv"
...
pairUUsam=$samdir/${RUN}.${pairlabel}.UU.bam
pairUxxUsam=$samdir/${RUN}.${pairlabel}.UxxU.bam
singUxxUsam=$samdir/${RUN}.${singlabel}.UxxU.bam
noneUxxUsam=$samdir/${RUN}.${nonelabel}.UxxU.bam
noneUUsam=$samdir/${RUN}.${nonelabel}.UU.bam
...
echo -e "PET_UU_q30\t"${pairUUcount} >> ${out_file}
echo -e "NoLinker_UU_q30\t"${noneUUcount} >> ${out_file}
echo -e "PET_Ux_q30\t"${pairUxCB} >> ${out_file}
echo -e "NoLinker_Ux_q30\t"${noneUxCB} >> ${out_file}
echo -e "1tag_Ux_q30\t"${singUxCB} >> ${out_file}

pairMarkdupFile=${RUN}.${pairlabel}.UU.F2048.CB.nr.MarkDuplicates.txt #consistent with singlelinker.paired.UU.F2048.CB.PET.tsv
uxxuMarkdupFile=${RUN}.${pairlabel}.UxxU.F2048.CB.nr.MarkDuplicates.txt #
singMarkdupFile=${RUN}.${singlabel}.UxxU.F2048.CB.nr.MarkDuplicates.txt
noneMarkdupFile=${RUN}.${nonelabel}.UU.F2048.CB.nr.MarkDuplicates.txt
noneUxxUMarkdupFile=${RUN}.${nonelabel}.UxxU.F2048.CB.nr.MarkDuplicates.txt
...
echo -e "PET_UU_q30_validCB\t"${pairExam} >> ${out_file} #consistent with CBcounts.tsv without noCB e.g.: $(awk 'NR>1 && $1 != "noCB" {sum +=$5} END {print sum}' $cbtsv | xargs printf "%'.f")
echo -e "PET_Ux_q30_validCB\t"${uxxuExam} >> ${out_file}
echo -e "1tag_Ux_q30_validCB\t"${singExam} >> ${out_file}
echo -e "NoLinker_UU_q30_validCB\t"${noneExam} >> ${out_file}
echo -e "NoLinker_Ux_q30_validCB\t"${noneUxxUExam} >> ${out_file}
...
echo -e "Redundancy_PET_UU\t"${uuPairDupRate} >> ${out_file} 
echo -e "Redundancy_PET_UxxU\t"${uxxuPairDupRate} >> ${out_file} 
echo -e "Redundancy_1tag_UxxU\t"${uxxuSingDupRate} >> ${out_file} 
echo -e "Redundancy_NoLinker_UU\t"${uuNoneDupRate} >> ${out_file} 
echo -e "Redundancy_NoLinker_UxxU\t"${uxxuNoneDupRate} >> ${out_file} 

nrPETuu=$(awk 'NR>1 && $1 != "noCB" {sum +=$18} END {print sum}' $cbtsv | xargs printf "%'.f")
echo -e "nrPET_UU\t"${nrPETuu} >> ${out_file} 

nrPETtra=$(awk 'NR>1 && $1 != "noCB" {sum +=$19} END {print sum}' $cbtsv)
printf  "nrPET_trans\t%'.f\n" ${nrPETtra} >> ${out_file} 
...
## Singleton
cis_file=${RUN}.*cis.chiasig.gz
tra_file=${RUN}.*trans.chiasig.gz
waitFile $cis_file
waitFile $tra_file
# Get singleton PET count (all)
singleton=$(zcat ${RUN}.*chiasig.gz | awk '$7==1{print}' | wc -l)
...
## Clusters (overall)
# Get cluster count
total_cluster_number=$(zcat ${RUN}.*chiasig.gz | awk '$7 > 1 {print}' | wc -l)
